...
public class MainIT {
    ...
    public void testSplitMapCommandlineArgs() {
        ...
        assertEquals(
                new AbstractMap.SimpleEntry<>("foo", Collections.singletonMap("bar", "tar")),
                Main.splitMap2("foo:bar=tar"));

        assertEquals(new AbstractMap.SimpleEntry<>("hello", Collections.emptyMap()), Main.splitMap2("hello"));
    ...
    public void testParseFelixFwVersion() {
        ...
        Main.parseArgs(noActionAllowesConfig, new String[] {"-" + Main.OPT_FELIX_FRAMEWORK_VERSION});
    ...
    public void testParseOSGiFwArtifact() {
        ...
        Main.parseArgs(noActionAllowesConfig, new String[] {"-" + Main.OPT_OSGI_FRAMEWORK_ARTIFACT});
    ...
    public void testParse_FW_Prop() {
        ...
        assertEquals(
                "1",
                configSpaceSingle.getInstallation().getFrameworkProperties().get("a"));

        LauncherConfig configSpaceNoSeparator = new LauncherConfig();
        Main.parseArgs(configSpaceNoSeparator, args("-D a=1,b=2"));

        assertTrue(configSpaceNoSeparator
                .getInstallation()
                .getFrameworkProperties()
                .containsKey("a"));
        assertEquals(
                "1,b=2",
                configSpaceNoSeparator
                        .getInstallation()
                        .getFrameworkProperties()
                        .get("a"));
        ...
        assertTrue(
                configSpaceMultiple.getInstallation().getFrameworkProperties().containsKey("a"));
        assertEquals(
                "1",
                configSpaceMultiple.getInstallation().getFrameworkProperties().get("a"));
        assertTrue(
                configSpaceMultiple.getInstallation().getFrameworkProperties().containsKey("b"));
        assertEquals(
                "2",
                configSpaceMultiple.getInstallation().getFrameworkProperties().get("b"));
    }
    ...
    public void testParse_Artifact_Clash() {
        ...
        config = new LauncherConfig();
        Main.parseArgs(config, new String[] {"-" + Main.OPT_ARTICACT_CLASH, "foo:bar:1", "foo:bar:2"});
        assertTrue(config.getArtifactClashOverrides().contains(ArtifactId.parse("foo:bar:1")));
        ...
        Main.parseArgs(
                config,
                new String[] {"-" + Main.OPT_ARTICACT_CLASH, "foo:bar:1", "-" + Main.OPT_ARTICACT_CLASH, "foo:bar:2"});
        assertTrue(config.getArtifactClashOverrides().contains(ArtifactId.parse("foo:bar:1")));
        assertTrue(config.getArtifactClashOverrides().contains(ArtifactId.parse("foo:bar:2")));
    }
    ...
    public void testParse_RepoUrls() {
        ...
        config = new LauncherConfig();
        Main.parseArgs(
                config, new String[] {"-" + Main.OPT_REPOSITORY_URLS, "foo", "-" + Main.OPT_REPOSITORY_URLS, "bar"});
        assertArrayEquals(config.getRepositoryUrls(), new Object[] {"foo", "bar"});
    }
    ...
    public void testParse_FeatureFiles() {
        ...
        config = new LauncherConfig();
        Main.parseArgs(config, new String[] {"-" + Main.OPT_FEATURE_FILES, "foo", "-" + Main.OPT_FEATURE_FILES, "bar"});
        assertTrue(config.getFeatureFiles().contains("foo"));
        assertTrue(config.getFeatureFiles().contains("bar"));
    }
